...
import metodokde.UnidadMedida;
import metodokde.CapaPuntos;
import metodokde.Capa;
import metodokde.EntradaGenerica;
import metodokde.CalculadorDistribucion;
import metodokde.ManejadorColor;
...
public class VisorMapa extends JComponent implements MouseListener, MouseMotionListener, MouseWheelListener {
    ...
    private ArrayList<ArrayList<Punto>> listaCapa = new ArrayList<>();
                

    public VisorMapa(double anchoBanda, double resolucion, LinkedHashMap<String, Capa> mapaCapasKML,  Pixel[][] arrPixels, int cantDivisiones,ArrayList<Punto> listaPuntos,ArrayList<Punto> listaLugares) {
        ...
        listaCapa.add(listaPuntos);//dengue
        listaCapa.add(listaLugares);//fuente de agua
    ...
    private void cargarKDE() {
        ...
        double[] arrExtremos = Utilidades.buscarCoordenadasExtremos(listaPuntos, this.anchoBanda);
    ...
    //Se obtienen los valores ordenados
        if (this.arrPixels != null) {
            ...
            this.arrColores = mc.obtenerEscalaColores("97F704", "F72828", this.cantDivisiones, false);
        }
        //Se obtienen los valores ordenados
        if (this.arrPixels != null) {
            CalculadorDistribucion cd = new CalculadorDistribucion();
            ...
            for (Map.Entry<String, Capa> entradaAux : this.mapaCapasKML.entrySet()) {
                ...
                switch (entradaAux.getValue().getTipoCapa()) {
                    ...
                    case 'p': //Capa de puntos
                        ...
                        CapaPuntos capaPuntos = new CapaPuntos(listaCapa.get(n));
                        //Se define el color de los puntos
                        if (listaCapa.get(n).equals(this.listaPuntos)) {
                            g.setColor(new Color(0, 32, 112));
                        } else {
                            g.setColor(new Color(117, 0, 61));
                        }
                        
                            
                            
                            
                            for (int j =0;j<listaCapa.get(n).size();j++) {
                            //Punto puntoAux = capaPuntos.getMapaPuntos().get(idPuntoAux);
                            Punto puntoAux = listaCapa.get(n).get(j);
                    ...
                    case 'a': //Capa de polígonos
                        CapaPoligono capaPoligonos = (CapaPoligono) entradaAux.getValue();
